...
use crate::metrics::{format_metrics_table, write_metrics_json, RunMetrics, StageTimer};
...
pub fn run_pipeline(args: RunArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Augment allowlist with alt-allele k-mers when provided.
    // Alt-allele sequences (generated by `multiseqex --alt-seq --flank N`)
    // span a known SNV or indel alt allele. Adding their k-mers to the allowlist
    // ensures that molecules carrying the alt sequence are captured during
    // indexing, boosting sensitivity at low VAF.
    if let Some(ref alt_path) = cfg.input.alt_as_ref {
        let alt_seqs = read_fasta(alt_path)?;
        let alt_slices: Vec<&[u8]> = alt_seqs.iter().map(|(_id, seq)| seq.as_slice()).collect();
        let alt_allowlist = build_allowlist(&alt_slices, k);
        let n_alt = alt_allowlist.len();
        allowlist.extend(alt_allowlist);
        log::info!(
            "[run/index] alt_as_ref: added {n_alt} alt-allele k-mers from {} sequences ({} total)",
            alt_seqs.len(),
            allowlist.len()
        );
    }
    ...
    let stage_index = timer_index.finish();
    log::info!(
        "[run/index] target_kmers={} observed={} elapsed_ms={}",
        n_target_kmers,
        n_kmers_observed,
        stage_index.elapsed_ms,
    );

    // ── Stage 3: Pathfind ────────────────────────────────────────────────────────
    let mut timer_pathfind = StageTimer::new("pathfind");
    ...
    // correct result.
    let graph_min_molecules = cfg
        .indexing
        .graph_min_molecules
        .expect("graph_min_molecules default-applied");
    let all_raw_kmers: Vec<u64> = raw_graph_index
        .iter()
        .filter(|(&km, _)| {
            index
                .get(canonical(km, k))
                .map(|e| e.n_molecules >= graph_min_molecules)
                .unwrap_or(false)
        })
        .map(|(&km, _)| km)
        .collect();
    ...
    let stage_pathfind = timer_pathfind.finish();
    log::info!(
        "[run/pathfind] targets={} with_variants={} no_paths={} ref_only={} alt_found={} start_missing={} end_missing={} soft_recovered={} elapsed_ms={}",
        n_targets_queried,
        n_targets_with_variants,
        n_walk_no_paths,
        n_walk_ref_only,
        ...
        stage_pathfind.elapsed_ms,
    );

    // ── Stage 4: Call ────────────────────────────────────────────────────────────
    let mut timer_call = StageTimer::new("call");
    ...
    if let Some(ref vcf_path) = cfg.input.target_variants {
        ...
        if let Some(ref alt_path) = cfg.input.alt_as_ref {
            // Sequence-level fallback: load alt sequences from the same FASTA
            // used for allowlist augmentation and pass them to the extended filter.
    ...
    if cfg.calling.alt_walk {
            ...
            (&cfg.input.target_variants, &cfg.input.alt_as_ref)
        {
            ...
            let alt_entries = load_alt_walk_entries(alt_path)
    ...
    let stage_call = timer_call.finish();
    log::info!(
        "[run/call] variants={} pass={} filtered={} elapsed_ms={}",
        n_variants_called,
        n_pass,
        n_filtered,
        stage_call.elapsed_ms,
    );

    // ── Stage 5: Output ─────────────────────────────────────────────────────────
    let mut timer_output = StageTimer::new("output");
    ...
    let stage_output = timer_output.finish();
    log::info!(
        "[run/output] elapsed_ms={} total_ms={}",
        stage_output.elapsed_ms,
    ...
    let total_elapsed = t_total.elapsed().as_millis() as u64;
    ...
    if metrics_enabled {
        ...
        write_metrics_json(&run_metrics, &metrics_path)?;
    }
...
fn build_config(args: &RunArgs) -> Result<KamConfig, Box<dyn std::error::Error>> {
    ...
    if cfg.indexing.graph_min_molecules.is_none() {
        cfg.indexing.graph_min_molecules = Some(2);
    }
