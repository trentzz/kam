...
pub struct IndexArgs {
    ...
    /// FASTA of alt-allele sequences (generated by `multiseqex --alt-seq --flank N`) to add
    /// alt k-mers to the allowlist. Enables alt-as-ref sensitivity boost for SNVs and indels.
    #[arg(long)]
    ...
    /// Write a TSV of indexed k-mers to this path for comparison with external tools (e.g. Jellyfish).
    ///
    /// Each row is one k-mer in the filtered index. Columns: kmer_seq, kmer_u64,
    /// n_molecules, n_duplex, n_simplex_fwd, n_simplex_rev, min_base_error_prob,
    /// mean_base_error_prob. Rows are sorted by kmer_u64 for deterministic output.
    #[arg(long)]
    pub dump_kmer_index: Option<PathBuf>,
}
...
pub struct CallArgs {
    ...
    /// FASTA of alt-allele sequences for sequence-level tumour-informed matching.
    ///
    /// When provided alongside `--target-variants`, calls whose full alt-path
    /// sequence exactly matches a sequence in this FASTA are marked PASS even if
    /// VCF tuple matching fails. This recovers indels in homopolymer runs where
    /// left-normalisation shifts further than the target window allows.
    ///
    /// Use the same file passed to `index --alt-as-ref` (generated by
    /// `multiseqex --alt-seq --flank N`). Header format: `chrN:start-end REF=... ALT=...`.
    #[arg(long)]
    pub alt_as_ref: Option<PathBuf>,
...
pub struct RunArgs {
    ...
    /// the de Bruijn graph used for pathfinding.
    ///
    /// K-mers whose canonical form is supported by fewer than this many
    /// molecules are dropped from the graph. Setting this to 1 allows
    /// singleton k-mers to survive, which can rescue indels at very low VAF
    /// where Poisson noise in alt-molecule counts at junction k-mers would
    /// otherwise break the alt path chain, but also increases graph size and
    /// the risk of error-driven spurious paths. Default: 2.
    #[arg(long)]
