...
In \emph{tumour-informed monitoring mode} (\texttt{--target-variants}), a VCF of expected somatic variants is provided and the caller only reports calls whose (CHROM, POS, REF, ALT) matches an entry in that set.
Discovery mode is appropriate for initial somatic profiling; monitoring mode is appropriate for serial liquid biopsy where the variant panel is known from a prior biopsy.

On the Twist cfDNA Pan-Cancer Reference Standard v2 titration series with tumour-informed filtering applied, kam detects 52--61\% of truth variants at 2\% VAF with precision 1.0 (zero false positives), running in 41--55 seconds per sample on a single core at 2M read pairs.
