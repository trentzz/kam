...
kam extends the de Bruijn graph framework to detect large structural variants (SVs) using junction k-mer injection. When the \texttt{--sv-junctions} flag is provided, a FASTA of SV breakpoint junction sequences is loaded during k-mer indexing. The junction k-mers are added to the allowlist alongside the panel target k-mers, making them eligible for inclusion in the de Bruijn graph.

Junction sequences span each SV breakpoint: for a deletion, the sequence joining the left flank of the deleted region to the right flank; for a tandem duplication, the head-to-tail junction; for an inversion, both the left and right inversion junctions. Each junction sequence is 60--80\,bp, long enough to provide $\ge 30$ overlapping 31-mers across the breakpoint. These k-mers create a continuous path through the graph from the upstream anchor to the downstream anchor via the SV allele, enabling path discovery for large events that would otherwise have no connecting path.
