...
Stage~4 (\textbf{Pathfind}) constructs a de~Bruijn graph from the index and enumerates all paths between reference anchor k-mers.
...
% Figure~\ref{fig:pipeline} would show a horizontal flow diagram:
...
kam builds a de~Bruijn graph~\autocite{Iqbal12cortex} from the k-mer index.
Nodes are canonical k-mers.
...
Algorithm~\ref{alg:bfs} describes the BFS path enumeration.

\begin{algorithm}
\caption{BFS path enumeration between anchor k-mers.}
\label{alg:bfs}
\begin{algorithmic}[1]
...
\While{$Q$ is not empty \textbf{and} $|\mathcal{P}| < P$}
  \State $\pi \gets Q.\text{dequeue}()$
  \State $v \gets \text{last}(\pi)$
  \For{each successor $u$ of $v$ in $G$}
    \If{$u \in \pi$} \Comment{Per-path cycle detection}
    ...
    \EndIf
    \If{$u = t$}
      \State $\mathcal{P} \gets \mathcal{P} \cup \{\pi'\}$
    \Else
      \State $Q.\text{enqueue}(\pi')$
    \EndIf
  \EndFor
\EndWhile
...
Each path maintains its own visited set, so the same k-mer can appear in different paths but not twice within a single path.
The default bounds are $L = 500$ k-mers and $P = 100$ paths.
...
\begin{itemize}
  ...
  \item Total forward and reverse simplex molecule counts (summed across k-mers).
\end{itemize}
...
Let $k$ be the number of molecules supporting the alternate path (from $n_\text{min}$ of the alt path) and $M$ be the total molecules ($n_\text{min,ref} + n_\text{min,alt}$).
...
Observing $k$ successes in $M$ trials yields posterior $\text{Beta}(k + 1, M - k + 1)$:

\begin{equation}
  \label{eq:vaf-ci}
  \text{CI}_{95\%} = \bigl[q_{0.025}, \; q_{0.975}\bigr]
  \quad \text{where} \quad
  q_\alpha = F^{-1}_{\text{Beta}(k+1, \, M-k+1)}(\alpha)
\end{equation}
...
Let $a$, $b$ be the number of forward and reverse simplex molecules supporting the alternate allele, and $c$, $d$ be the corresponding counts for the reference allele:
...
\noindent where $L(p) = p^k (1 - p)^{M - k}$ is the binomial likelihood and $\varepsilon$ is the per-site background error rate (default: $10^{-4}$).
...
A variant passes all filters when: the posterior confidence $\ge 0.99$, the strand bias p-value $\ge 0.01$, the alt molecule count $\ge 2$ (configurable), and the duplex alt molecule count meets the minimum threshold (default: 0).
Variants failing any criterion are labelled with the corresponding filter (\texttt{LowConfidence}, \texttt{StrandBias}, \texttt{LowDuplex}, or \texttt{CollisionRisk}).
