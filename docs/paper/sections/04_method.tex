...
\begin{itemize}
  \item $|R| - |A| \ge 50$: \textit{LargeDeletion}.
  \item $|A| - |R| \ge 50$: \textit{TandemDuplication}.
...
SV calls use type-specific thresholds. The default confidence threshold for SNVs and indels is 0.99; for SV types the threshold is 0.95 (\texttt{--sv-min-confidence}), and the minimum alt molecule count is 1 (\texttt{--sv-min-alt-molecules}). A large structural event with even 2 supporting molecules carries qualitatively stronger signal than 2 molecules of a single-base substitution: background sequencing errors cannot produce a 50--200\,bp structural signature. The duplex requirement is relaxed by default for SV benchmarking (\texttt{--min-duplex-alt-molecules 0}), as synthetic SV simulations may produce only simplex molecule evidence. In tumour-informed mode (\texttt{--target-variants}), the SV allele representation in the target VCF must match the key extracted by the pipeline; a Python helper (\texttt{make\_monitoring\_vcf.py}) generates tumour-informed VCFs directly from discovery results to guarantee exact key matching.
