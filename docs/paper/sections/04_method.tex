...
SV calls pass through the same confidence and filter pipeline as SNVs and indels. The duplex requirement is relaxed by default for SV benchmarking (\texttt{--min-duplex-alt-molecules 0}), as synthetic SV simulations may produce only simplex molecule evidence. In monitoring mode (\texttt{--target-variants}), the SV allele representation in the target VCF must match the key extracted by the pipeline; a Python helper (\texttt{make\_monitoring\_vcf.py}) generates monitoring VCFs directly from discovery results to guarantee exact key matching.
