...
```
```
docs/project/experiments/01-ml-twist-duplex/   ← tracked in git
├── configs/
...
├── checkpoint.json     # Pipeline checkpoint state (current batch progress)
├── design.md           # Dataset design and model methodology
├── models/             # (Nextcloud) trained model files
│   ├── lightgbm_twist.txt
│   ├── xgboost_twist.json
│   ├── ensemble_twist.pkl
│   └── xgboost_twist.onnx
...
bigdata/experiments/01-ml-twist-duplex/         ← gitignored, mirrors Nextcloud
├── simulations/
│   ├── train/
│   │   └── {sample_name}/
│   │       ├── *_R1.fastq.gz
...
Large files (simulations, feature CSVs, models) are not tracked in git. They live in `bigdata/experiments/01-ml-twist-duplex/` locally and on Nextcloud under `experiments/01-ml-twist-duplex/`. See [docs/project/devmanual/nextcloud.md](../../devmanual/nextcloud.md).
...
This writes 11,000 YAML configs to `docs/project/experiments/01-ml-twist-duplex/configs/`
and 11,000 truth VCFs to `docs/project/experiments/01-ml-twist-duplex/truth_vcfs/`.
