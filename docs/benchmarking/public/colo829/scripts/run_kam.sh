...
#
# Prerequisites:
#   - Download data:         sh docs/benchmarking/public/scripts/download_colo829.sh
...
#   docs/benchmarking/public/colo829/results/

set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)"
KAM="${REPO}/target/release/kam"
SCRIPTDIR="${REPO}/docs/benchmarking/public/colo829"
...
echo "          Track progress in docs/benchmarking/public/colo829/README.md." >&2
echo "" >&2
...
for f in "$TUMOUR_R1" "$TUMOUR_R2"; do
    if [[ ! -f "$f" ]]; then
        ...
        echo "        Run: sh ${REPO}/docs/benchmarking/public/scripts/download_colo829.sh" >&2
        exit 1
    fi
done
...
echo ""
echo "See docs/benchmarking/public/colo829/README.md for full evaluation notes."
