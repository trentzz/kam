...
#   See docs/benchmarking/public/seqc2/README.md.
#
# Prerequisites:
#   - Download data:   sh docs/benchmarking/public/scripts/download_seqc2.sh
...
#   docs/benchmarking/public/seqc2/results/

set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)"
KAM="${REPO}/target/release/kam"
SCRIPTDIR="${REPO}/docs/benchmarking/public/seqc2"
...
echo "          Track progress in docs/benchmarking/public/seqc2/README.md." >&2
echo "" >&2
...
for f in "$TUMOUR_R1" "$TUMOUR_R2" "$NORMAL_R1" "$NORMAL_R2"; do
    if [[ ! -f "$f" ]]; then
        ...
        echo "        Run: sh ${REPO}/docs/benchmarking/public/scripts/download_seqc2.sh" >&2
        exit 1
    fi
done
...
echo "See docs/benchmarking/public/seqc2/README.md for full evaluation steps."
