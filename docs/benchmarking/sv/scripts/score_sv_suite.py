#!/usr/bin/env python3
"""Score all SV benchmark datasets (DUP+INV, INS, INVDEL).

Scoring uses POSITION-BASED matching with a ±10bp tolerance window. This is
necessary because kam reports partial allele representations for large SVs
(100bp insertions/inversions) rather than the full allele sequences. REF/ALT
sequence matching would always produce 0% sensitivity for SVs.

For each truth variant:
  - A call is a TP if any PASS call has POS within ±10bp of the truth POS.
  - Multiple PASS calls near the same truth position count as one TP.

Outputs:
  docs/benchmarking/sv/summary/sv_per_dataset.tsv
  docs/benchmarking/sv/summary/sv_summary.tsv
...
def score_positions(truth_positions: List[int], called_positions: List[int], tol: int = POSITION_TOLERANCE) -> Dict:
    ...
    return {"tp": tp, "fp": fp, "fn": fn, "sensitivity": sensitivity, "precision": precision, "f1": f1}


PER_DS_COLS = ["type", "vaf", "replicate", "mode", "tp", "fp", "fn", "sensitivity", "precision", "f1"]
...
def main() -> None:
    ...
    for sv_type in SV_TYPES:
        for vaf in VAF_LEVELS:
            t = vaf_tag(vaf)
            for rep in ["a", "b"]:
                sim_dir = ROOT / "results" / f"sim_{sv_type}_vaf{t}_{rep}"
    ...
    per_ds_path = SUMMARY_DIR / "sv_per_dataset.tsv"
