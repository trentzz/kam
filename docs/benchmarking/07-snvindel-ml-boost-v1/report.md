...
**Date:** 2026-04-12 (updated; original run 2026-04-11)
...
- **Precision** is 1.000 across all conditions and all samples. The baseline achieves this because discovery mode does not generate spurious calls on this panel dataset. TI mode achieves precision = 1.000 by construction.
...
- **Sensitivity is slightly higher** under TI-only and ML+TI than under the baseline at most VAF levels (up to 4.3% absolute at 1% VAF for 5 ng). This reflects binary improvements between the baseline build and the current build, not the TI mechanism itself.
- **A targeting.rs normalisation regression** (introduced 2026-04-06, fixed 2026-04-12) caused indel sensitivity to drop from ~67 TPs to ~23 TPs at 15 ng 2% VAF. The data in this report uses corrected results. See `docs/project/investigations/05-kam-gap-indels.md` for details.
...
- Binary: `target/release/kam` from the commit immediately before ML work began (pre-squash, no normalisation regression).
- No `--target-variants` flag.
- No ML filter.
- Scoring: Python script matching against the truth VCF using the same left-normalisation algorithm as the Rust code.
- Source TSV: `docs/benchmarking/01-snvindel/summary/titration_results_2mreads.tsv`.

#### TI-only

- Binary: `target/release/kam --features ml` (current build, targeting.rs normalisation regression fixed 2026-04-12).
- Flag: `--target-variants docs/benchmarking/01-snvindel/scripts/truth_variants.vcf`.
...
The truth VCF (`docs/benchmarking/01-snvindel/scripts/truth_variants.vcf`) uses standard 1-based VCF coordinates. The Rust `--target-variants` implementation reads positions from the VCF directly and matches them against variant positions computed by `extract_variant_key` in `kam-call/src/targeting.rs`. For a base at window index `i` within a 100 bp target window with 1-based start coordinate `target_start`, the correct VCF position is `target_start + i` (no offset).

A `truth_variants_1based.vcf` file exists in the benchmark scripts directory. It has every position incremented by 1 relative to the standard VCF. This was created as a workaround for a regression introduced in a squash commit (`0865391b`, 2026-04-06) that added an extraneous `+ 1` to all position formulas in `targeting.rs`. The regression caused the Rust TI filter to compute positions one higher than the truth VCF, causing all calls to receive `NotTargeted` status. The `truth_variants_1based.vcf` compensated for the regression by shifting truth positions to match the buggy code. This workaround is now superseded.

The fix (2026-04-12) removed the `+ 1` from all position formulas in `targeting.rs` and `run_titration_batch.py`. The TI runs in this report used `truth_variants.vcf` with the corrected binary, producing the values shown. The `truth_variants_1based.vcf` file is retained for reference but should not be used. See `docs/project/investigations/05-kam-gap-indels.md` for the full investigation.
