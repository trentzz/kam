...
#
# Prerequisites:
#   - Download data with: sh docs/benchmarking/05-public/scripts/download_umi_benchmark.sh
...
#   docs/benchmarking/05-public/umi_benchmark/results/

set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../../.." && pwd)"
KAM="${REPO}/target/release/kam"
SCRIPTDIR="${REPO}/docs/benchmarking/05-public/umi_benchmark"
...
if [[ ! -f "$R1" || ! -f "$R2" ]]; then
    ...
    echo "          sh ${REPO}/docs/benchmarking/05-public/scripts/download_umi_benchmark.sh" >&2
    echo "" >&2
...
echo "  - See docs/benchmarking/05-public/umi_benchmark/README.md for context."
